...
docs/      source release and security operations
release/   pinned runtime and maintainer trust metadata
scripts/   installer verification and shared shell logic
```
...
```bash
bash -n install.sh update.sh uninstall.sh scripts/lib/perch-common.sh
...
- never places secrets in arguments or environment variables;
...
`install.sh`, `update.sh`, and `uninstall.sh` are user-level, idempotent, and
source-first. Updates stage a verified signed tag, back up SQLite, atomically
activate runtime files, and roll back app, daemon, LaunchAgent, and database on
failure. Do not add `curl | bash`, Sparkle, auto-downloaded app archives, root
installation, or silent prerequisite installation.
